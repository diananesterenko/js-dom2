...
const newsListEl = createNewsListEl(news);
document.body.append(newsListEl);
function createNewsListEl(news) {
  const newsListEl = document.createElement("ul");
  newsListEl.classList.add("newsList");

  const newsCards = news.map((n) => createNewsEl(n));
  newsListEl.append(...newsCards);
  return newsListEl;
}
function createNewsEl(news) {
  const newsEl = document.createElement("li");
  newsEl.classList.add("news");
  newsEl.addEventListener("click", highlightNewHandler);
  function highlightNewHandler(e) {
    newsEl.classList.toggle("highlightLi");
  }
  newsEl.append(createNewsHeaderEl(news), createNewsContentEl(news));
  return newsEl;
}
function createNewsHeaderEl({ headerBgSrc, title }) {
  const newsHeaderEl = document.createElement("div");
  newsHeaderEl.classList.add("newsHeader");
  newsHeaderEl.style.backgroundImage = `url(${headerBgSrc})`;

  const newsHeaderBackgroundEl = document.createElement("div");
  newsHeaderBackgroundEl.classList.add("newsHeaderBackground");
  newsHeaderEl.append(newsHeaderBackgroundEl);

  const likeIconEl = createLikeIcon();
  newsHeaderBackgroundEl.append(likeIconEl);

  const newsTitleEl = document.createElement("h2");
  newsTitleEl.classList.add("newsTitle");
  newsTitleEl.textContent = title;
  newsHeaderBackgroundEl.append(newsTitleEl);
...
function createNewsContentEl({ category, body, date }) {
  const newsContentEl = document.createElement("div");
  newsContentEl.classList.add("newsContent");

  const newsContetntTitleEl = document.createElement("h5");
  newsContetntTitleEl.classList.add("newsContetntTitle");
  newsContetntTitleEl.textContent = category;
  newsContentEl.append(newsContetntTitleEl);

  const newsContentTextEl = document.createElement("p");
  newsContentTextEl.classList.add("newsContentText");
  newsContentTextEl.textContent = body;
  newsContentEl.append(newsContentTextEl);

  const newsContentDateEl = document.createElement("p");
  newsContentDateEl.classList.add("newsContentDate");
  newsContentDateEl.textContent = date;
  newsContentEl.append(newsContentDateEl);

  const deleteIconEl = createDeleteIcon();
  newsContentEl.append(deleteIconEl);

  return newsContentEl;
}
function createDeleteIcon() {
  const deleteIconEl = document.createElement("i");
  deleteIconEl.classList.add("fa-solid");
  deleteIconEl.classList.add("fa-trash");

  deleteIconEl.classList.add("delete");
  deleteIconEl.onclick = deleteNews;

  function deleteNews(e) {
    e.target.closest(".news").remove();
  }
  return deleteIconEl;
}
